...
test("Shrimpy TUI commands use public extension registration and custom UI", async () => {
  ...
  assert.match(panels[1]!, /\/settings\s+Edit durable Pi preferences/);
  assert.match(panels[1]!, /\/agents \[id\]\s+Navigate agents and local sessions/);
  assert.match(panels[1]!, /\/thinking\s+Select thinking; Ctrl\+S saves the agent default/);
  assert.match(panels[1]!, /\/changelog\s+Show the Shrimpy changelog/);
  assert.match(
    panels[1]!,
    /session defaults, model guardrails, and changelog\s+surface/,
  );
