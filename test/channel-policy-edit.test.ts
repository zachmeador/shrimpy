...
import { editChannelPolicyConfig } from "../dist/agents/channel-policy-edit.js";

describe("editChannelPolicyConfig", () => {
  test("sets a base field without disturbing siblings", () => {
    const next = editChannelPolicyConfig(
      { mode: "mentions" },
  ...
  test("deduplicates set values", () => {
    const next = editChannelPolicyConfig(undefined, {
  ...
  test("returns null when the last field is cleared", () => {
    const next = editChannelPolicyConfig({ mode: "mentions" }, { clear: ["mode"] });
  ...
  test("does not mutate the input config", () => {
    ...
    editChannelPolicyConfig(current, { channel: "ops", set: { senders: ["human"] } });
