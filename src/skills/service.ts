...
export function listEffectiveSkillEntryPathsFromPaths(opts: {
  ...
  return scanSkillEntries(opts).entries.map((entry) => entry.entryPath);
}
...
function largeSkillSetWarnings(skills: SkillView[]): string[] {
  ...
  return [
    `${visibleCount} effective skills will be advertised to Pi; consider splitting or removing unused workspace skills if prompt context gets noisy.`,
  ];
}
