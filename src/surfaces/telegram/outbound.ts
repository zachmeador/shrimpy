...
export async function sendTelegramFormattedText(
  telegram: Pick<{ sendMessage(chatId: number, text: string, parseMode?: string): Promise<void> }, "sendMessage">,
  chatId: number,
  text: string,
): Promise<void> {
  for (const chunk of renderTelegramTextChunks(text)) {
    try {
      await telegram.sendMessage(chatId, chunk.text, chunk.parseMode);
    } catch (err) {
      ...
      for (const plainChunk of splitPlainTextChunks(text.trim(), TELEGRAM_TEXT_CHUNK_LIMIT)) {
        ...
        if (plainText) await telegram.sendMessage(chatId, plainText);
      }
      return;
    }
  }
