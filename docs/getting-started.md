...
This guide is for a first, curious setup. Setup choices depend on the providers available in your installation.
...
Setup shows available subscription, API-key, and local-model options. Choose an account you already use and read any provider notice shown during sign-in, including usage limits or billing conditions.
...
For a quick, supervised look, using your normal Linux or macOS account can be reasonable if you keep scheduled agents disabled, stay present while the agent works, and give it only folders you are comfortable letting it touch. Setup asks which optional routines you want and creates only those selected. It asks before enabling them or starting the background gateway.
...
Setup creates the basic workspace, then checks model access. If no working model is available, choose a local endpoint, API key, or subscription login from the model-access menu and follow its instructions.

When setup offers model candidates, select the one you want for normal work. It saves that choice in the `coding` model policy so you do not have to repeat provider details in every conversation.
...
At the end, setup prints the next commands and the workspace, config, command, and log paths.
...
You are talking to the normal `shrimpy` agent. It is fine to begin with plain requests:
...
Use `shrimpy chat mechanic` whenever you want help with models, tools, agents, skills, channels, schedules, updates, or repairs. Use `shrimpy chat shrimpy` to select your main agent explicitly. Plain `shrimpy` resumes the agent with the most recent terminal-chat activity.
...
Setup offers optional memory upkeep, journaling, and read-only security and hygiene audits. It creates only the routines you select. Each run uses your configured model, so start with a routine that serves a real need.

Open `shrimpy chat mechanic` and ask it to inspect existing watches and propose the routines you want. The [default-watch skill](../src/skills/included/shrimpy-watches-default-init/SKILL.md) defines their owners, cadences, and messages. If you skipped a routine during setup, ask the mechanic to create it before enabling it.

Inspect the result with `shrimpy watches` and `shrimpy gateway status`. Scheduled work runs only while the gateway is running.
