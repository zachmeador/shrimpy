...
## Wave 3 — type-surface collapse — done 2026-06-09

Dropped declaration output for the bin-only package, collapsed channel inspection/source DTOs into one inspected-message shape, skipped channel activity inspection for `channels list` text mode, expressed exact context derivations as aliases, localized watches/session/project-sync DTOs, and mechanically unexported zero-import internal type declarations while keeping Telegram wire types, channel protocol types, and Config/Resolved contracts. Exported interface/type count is 210 (target ≤285); `npm run lint`, `npx tsc --noEmit`, and `npm test` (479 pass) are green.

Decision gate outcome: declaration output was dropped because shrimpy is a bin-only CLI and nothing consumes its `.d.ts` files. If declaration output returns later, "kill" means inline/merge, not unexport.
