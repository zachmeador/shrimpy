...
Agents need a dependable answer to: "What do I know right now, why do I know it, and where can I inspect the source?" Shrimpy already has the shape of this in `ContextBlock`, but session prompt sections, turn context items, and CLI source inspection still flow through separate paths.
...
This is lowest-priority observability work. Build it only when context provenance becomes a recurring debugging problem, or when continuation, watch, and worker-status context expands enough that the existing `shrimpy context --sections`, `shrimpy context turn`, and `shrimpy context sources list/run` surfaces are no longer sufficient.
...
- Include the durable user message and provider-facing injected context view in turn traces, without persisting the injected context as a transcript message.
- Keep command and runtime producers explicitly inspectable without duplicating execution logic in the CLI.

## Boundaries

- Do not create a second prompt assembly path.
- Do not make the trace a persistent memory store or control plane.
- Keep planning separate from materialization, so list/section views do not accidentally execute command sources or mutate turn context freshness state.
- Preserve stable prompt resources as session context and live facts as turn context.
- Preserve the stable session-context boundary: trace/debug views may show provider-facing injected context, but they must not turn it back into durable prompt text.
- Do not add legacy compatibility paths unless a concrete workspace-facing break requires it.
...
- Tests cover prompt/trace parity, turn context/trace parity, source listing, source rendering, command freshness behavior, and skipped/failed sources.
