...
security-owned watch, Markdown reports, a compact active-systems inventory, and
a short TLDR. The agent recommends and explains. It does not silently fix,
disable, update, delete, or migrate anything.
...
- Seed a weekly security-owned watch. The watch records audit run history and
  emits into a meaningful user/security channel only when there are findings,
  useful no-op summaries, or actionable failures.
...
- Shrimpy's security scope includes any app, repo, script, service, watch,
  browser profile, workflow, or host integration that a Shrimpy agent has built,
  modified, configured, watched, or agreed to maintain.
- The default audit starts with Shrimpy state: config, channels, watches,
...
- Existing watch, channel, skill, and agent primitives should be enough for a
...
- The audit never changes config, code, services, watches, or dependencies on
  its own.
- Tests cover setup output, watch seeding, skill validation, and report-path
