...
`ContextBlock`, but session prompt sections, turn briefing items, and CLI source
inspection still flow through separate paths.

Make context assembly inspectable as one deterministic trace:

```text
source config -> resolved source plan -> produced context blocks -> rendered prompt/briefing
```

The inspection view should match the actual material used for the prompt and
turn briefing, so agents and developers can trust `shrimpy context` output when
debugging a session.
...
- Keep planning separate from materialization, so list/section views do not
  accidentally execute command sources or mutate briefing freshness state.
...
- This should make CTX-003 and CTX-007 easier to add because new briefing facts
...
- Context source resolution, block materialization, prompt rendering, turn
  briefing rendering, and CLI inspection share one trace model.
...
- Tests cover prompt/trace parity, turn briefing/trace parity, source listing,
