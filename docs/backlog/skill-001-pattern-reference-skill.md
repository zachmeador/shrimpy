...
- Neighboring backlog notes own the deep versions of two patterns: [SECURITY-002](security-002-default-security-audit-agent.md) (bundled security audit agent) and [MECH-001](mech-001-skill-opportunity-watch.md) (mechanic assessment watch and anti-pattern catalog).
...
- Remove `shrimpy-mechanic-ideas` entirely: defaults entry, template directory, and the pointer in the `mechanic` skill. Fold its "how to run a recommendations session" remainder into the `mechanic` skill or leave it to [MECH-001](mech-001-skill-opportunity-watch.md).
