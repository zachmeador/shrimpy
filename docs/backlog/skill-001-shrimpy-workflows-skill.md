...
- Source-default all-agent skills already cover upkeep and coding handoff, but no all-agent skill tells a normal agent how to choose between watches, channels, vault files, memory search, and workers.
...
- Keep `SKILL.md` short. It should tell agents to check the relevant pattern first, then the relevant reference docs if they need more detail. It should not copy command catalogs or long examples.
...
- Keep each pattern doc short, command-backed, and named after the Shrimpy concept it explains. Pattern docs hold shared workflow knowledge; skills point to them and add intended agent behavior.
...
- The skill description and body route ordinary workflow requests to the correct pattern doc first, then reference docs when more detail is needed.
- Five backing docs exist under `docs/patterns/`: `watches.md`, `channels.md`, `vault.md`, `memory.md`, and `workers.md`.
