...
`shrimpy context` is supposed to answer "what exact context will this agent/model turn see?" Today it still uses separate inspection assembly paths and can miss material that real sessions pass through Pi/provider adapters. That is a correctness bug in the inspection command, not a separate designed context surface.
