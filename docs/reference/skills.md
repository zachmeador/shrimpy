...
Default setup no longer copies unchanged Shrimpy default skills into the workspace. Default upkeep skills such as `memory-management`, `journal-daily`, and `journal-compact` plus workflow routing through `shrimpy-workflows`, coding handoff guidance through `coding-delegation`, web lookup delegation through `codex-web-search`, and vault capture guidance through `vault-capture` are source defaults available to all compatible agents. Maintenance capabilities such as `mechanic`, `add-agent`, `channel-routing`, `watches`, `skill-management`, `workspace-migration`, `setup`, `security-audit`, and `hygiene-audit` are source defaults visible only to the `mechanic` agent. Shared Shrimpy workflow knowledge belongs in `docs/patterns/` or `docs/reference/`; skills should point there and keep only trigger scope, intended agent behavior, and safety rules. A user or agent can still add local files under `skills/` or `agents/<id>/skills/` to customize or add capabilities.
